...
<body class="bg-gray-100">
    ...
    <div class="w-full max-w-7xl mx-auto p-4 sm:p-6 lg:p-8">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
         <div class="bg-white rounded-lg border border-gray-200 shadow-sm"> <div class="p-4 pb-2"> <h3 class="text-sm font-medium text-gray-600">Total Users</h3> </div> <div class="px-4 pb-4"> <div class="text-2xl font-bold"> <?php echo number_format($stats['totalUsers']); ?> </div> </div> </div>
         <div class="bg-white rounded-lg border border-gray-200 shadow-sm"> <div class="p-4 pb-2"> <h3 class="text-sm font-medium text-gray-600">Total Orders</h3> </div> <div class="px-4 pb-4"> <div class="text-2xl font-bold"> <?php echo number_format($stats['totalOrders']); ?> </div> </div> </div>
         <div class="bg-white rounded-lg border border-gray-200 shadow-sm"> <div class="p-4 pb-2"> <h3 class="text-sm font-medium text-gray-600">Products</h3> </div> <div class="px-4 pb-4"> <div class="text-2xl font-bold"><?php echo number_format($stats['totalProducts']); ?></div> </div> </div>
         <div class="bg-white rounded-lg border border-gray-200 shadow-sm"> <div class="p-4 pb-2"> <h3 class="text-sm font-medium text-gray-600">Revenue (Paid)</h3> </div> <div class="px-4 pb-4"> <div class="text-2xl font-bold"> ₹<?php echo number_format($stats['revenue']); ?> </div> </div> </div>
      </div>
      ...
      <div class="w-full">
        ...
        <?php if ($activeTab === 'orders'): ?>
             <div class="bg-white rounded-lg border border-gray-200 shadow-sm">
                ...
                <div class="p-6">
                  <div class="space-y-4">
                     <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4"> <div> <h3 class="font-medium">Recent Orders</h3> <p class="text-sm text-gray-600"> Latest customer orders </p> </div> <button class="inline-flex items-center px-4 py-2 border border-gray-300 text-gray-700 text-sm font-medium rounded-md hover:bg-gray-50 transition-colors"> <i class="fas fa-file-export mr-2"></i> Export Orders </button> </div>
                    <div class="border rounded-lg overflow-x-auto">
                      <table class="w-full min-w-full divide-y divide-gray-200">
                          ...
                          <?php foreach ($orders_from_db as $order): ?>
                            <tr id="order-row-<?php echo $order['id']; ?>" class="hover:bg-gray-50">
                              ...
                              <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium relative">
                                   ...
                                   <div id="dropdown-order-<?php echo $order['id']; ?>" class="action-dropdown w-48 bg-white rounded-md shadow-lg border border-gray-200 hidden">
                                     <div class="py-1" role="menu" aria-orientation="vertical">
                                         <button onclick="openEditModal('order', <?php echo $order['id']; ?>)" class="flex items-center w-full px-4 py-2 text-sm text-gray-700 hover:bg-gray-100" role="menuitem"> <i class="fas fa-eye w-4 h-4 mr-2 text-gray-500"></i> View/Edit Details </button>
...
<?php endif; ?>
    ...
    <div id="editOrderModal" class="modal-overlay hidden">
         <div class="modal-content relative">
             ...
             <form id="editOrderForm" onsubmit="submitEditForm('order', event)" class="mt-6 pt-4 border-t">
                 <input type="hidden" id="editOrderId" name="order_id">
                 <div class="space-y-2">
                     <label for="editOrderStatus" class="form-label">Update Order Status</label>
                 ...
                 <div class="modal-actions" style="border-top: none; padding-top: 1rem;">
                     <button type="button" onclick="closeEditModal('order')" class="inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 sm:text-sm">Close</button>
                     <button type="submit" id="editOrderSaveButton" class="inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-blue-600 text-base font-medium text-white hover:bg-blue-700 sm:text-sm">Update Status</button>
                 </div>
    ...
    <script>
            ...
            try {
                ...
                if (result.success) {
                    ...
                    if (action.startsWith('delete_')) {
                        ...
                        if (row) {
                            row.querySelector('[data-status-badge]').textContent = data.order_status;
                            // You can add color logic for status here if you want
                        }
            ...
            if (id === null) { // "Add New" action
                ...
                if (data) {
                    ...
                    if(entity === 'user') {
                        ...
                        title.textContent = `Order Details #${data.id}`;
                        ...
                        if (data.items_json) {
                            try {
                                ...
                                items.forEach(item => {
                                    itemsHTML += `<div class="text-sm border-b pb-2 mb-2"><strong>${item.name || 'Product'}</strong> (Qty: ${item.qty})</div>`;
                                });
                            } catch(e) { itemsHTML = '<p class="text-red-500">Error reading order items.</p>'; }
                        }
                        
                        contentDiv.innerHTML = `
                            <h4 class="font-medium text-gray-800">Customer Details</h4>
                            ...
                            <h4 class="font-medium text-gray-800 mt-4">Items</h4>
                            ${itemsHTML || '<p class="text-sm text-gray-500">No items found in this order.</p>'}
                        `;
                    }
                    modal.classList.remove('hidden');
        ...
        async function submitEditForm(entity, event) {
            ...
            if (entity === 'user') {
                ...
                id = data.order_id;
                action = 'update_order_status'; // Specific action for this form
            }
